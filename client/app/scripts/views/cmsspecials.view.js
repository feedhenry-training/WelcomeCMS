/*global App, Backbone*/
/* Backbone View */
App.View.CMSSpecialsView = Backbone.View.extend({
  ...
  initialize: function(){
    this.collection = App.collections.specials;
    var self = this;
    this.listenTo(this.collection, 'reset', function () {console.log('specials change - calling render');self.render();});
  },

  render: function() {
    this.navbar = new App.View.NavbarView({title: "CMS Specials"});
    this.$el.empty();
    this.$el.append(this.navbar.render());

    var tpl = App.Templates.cmsspecials({specials: this.collection.toJSON()});
    console.log('tpl: ', tpl);
    this.$el.append(tpl);
    return this.$el;
  },
